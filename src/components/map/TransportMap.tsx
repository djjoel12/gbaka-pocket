...
type TransportMapProps = { points: GPSPoint[] };
...
export default function TransportMap({ points }: TransportMapProps) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl shadow-sm">
      <MapContainer center={currentPosition} zoom={13} scrollWheelZoom={true} className="h-[400px] w-full">
        <TileLayer
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        />
        <MapFollower position={currentPosition} />
        {lastPoint && <Marker position={currentPosition} icon={defaultIcon} />}
        {routePositions.length > 1 && <Polyline positions={routePositions} />}
        {/* Suppression du doublon MapFollower */}
      </MapContainer>
    </div>
  );
}
